...
int v4l2_s_fmt(int fd, int* width, int* height, unsigned int fmt, enum v4l2_buf_type type)
{
    ...
    pixfmt.field = V4L2_FIELD_NONE;//V4L2_FIELD_ANY;//V4L2_FIELD_NONE
...
int v4l2_querybuf(int fd, struct v4l2_buf* v4l2_buf)
{
    ...
    for(i = 0; i < v4l2_buf->nr_bufs; ++i)
    {
        ...
        buf_unit->bytesused = buf.bytesused;//for h264
        buf_unit->start = NULL;
    }
...
struct v4l2_buf_unit* v4l2_dqbuf(int fd, struct v4l2_buf* v4l2_buf)
{
    ...
    if (ioctl(fd, VIDIOC_DQBUF, &buffer) < 0)//读取数据帧
    {
        printf("ERR(%s):VIDIOC_DQBUF failed, dropped frame\n", __func__);
        return NULL;
    }
	//printf("buf.bytesused=%d\n",buffer.bytesused);
	v4l2_buf->buf[buffer.index].bytesused=buffer.bytesused;//新添加了bytesused元素，强制传递
...
int v4l2_s_bitrate(int fd, int h264_bitrate)
{
...
int v4l2_s_frameinterval(int fd, int interval)
{
	struct v4l2_control i_frame_interval;
	i_frame_interval.id = V4L2_CID_MPEG_VIDEO_H264_I_PERIOD;
    i_frame_interval.value = interval;
	if (ioctl(fd, VIDIOC_S_CTRL, &i_frame_interval) == -1) {
		printf("ERR(%s):v4l2_s_frameinterval failed\n", __func__);
		return -1;
	}
	return 0;
}
int v4l2_s_profile(int fd, int profile)
{
	struct v4l2_control control;
	control.id = V4L2_CID_MPEG_VIDEO_H264_PROFILE;
	control.value = V4L2_MPEG_VIDEO_H264_PROFILE_MAIN;
	if (ioctl(fd, VIDIOC_S_CTRL, &control) == -1) {
		printf("ERR(%s):v4l2_s_profile failed\n", __func__);
		return -1;
	}
	return 0;
}
//添加h264编码level控制：
int v4l2_s_level(int fd, int level){
	struct v4l2_control control;
	control.id = V4L2_CID_MPEG_VIDEO_H264_LEVEL;
	control.value = V4L2_MPEG_VIDEO_H264_LEVEL_4_0;
	if (ioctl(fd, VIDIOC_S_CTRL, &control) == -1) {
		printf("ERR(%s):v4l2_s_level failed\n", __func__);
		return -1;
	}
	return 0;
}
//添加h264编码sps/pps控制：
int v4l2_s_sps(int fd, int value){//0禁止在每个idr前，1为允许
	struct v4l2_control control;
