...
bool AlsaAACDevFramedSource::encode(uint8_t* pcmFrame, int samplingNum, Frame* frame)
{
    ...
    int ret;
	unsigned char* aacData = new unsigned char[mMaxOutputBytes]; //新增加临时变量，直接将frame->mFrame传入faacEncEncode出现段错误
    ret = faacEncEncode(mFaacEncHandle, (int*)pcmFrame, samplingNum, aacData, mMaxOutputBytes);
    if(ret <= 0)
        return false;

    timeval time;
    gettimeofday(&time, NULL);
    memcpy(frame->mFrame,aacData,ret);//采用复制方式
